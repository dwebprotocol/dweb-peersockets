...
Peersockets is a lightweight wrapper around [`@dwebvault/networker`](https://github.com/corestore-networker) that lets you exchange topic-tagged messages with peers over [`ddatabase-protocol`](https://github.com/hypercore-protocol) streams.
...
### Upgrading
If you've already been using peersockets v0, take a look at [`UPGRADE.md`](https://github.com/peersockets/blob/master/upgrade.md) to see what's changed in v1.
...
```
### Example
This example assumes you already have Networker ([`@dwebvault/networker`](https://github.com/corestore-swarm-networking)) instance.
```
...
* `networker` is an instance of [`@dwebvault/networker`](https://github.com/corestore-networker)).
...
`peer` is a `@dwebvault/networker` Peer instance that can be obtained from `networker.peers`.
